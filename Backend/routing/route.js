...
router.get('/:email',verifyToken, function(req,res){
    jwt.verify(req.token,'secretkey', (err, authData)=>{
        if(err){
            ...
            userService.getUser(req.params.email).then(data=>{res.json(data)})  }
    });
   
 });

 router.get('/findbyAccountId/:acct',verifyToken, function(req,res){
  
  userService.getUserByAccountId(req.params.acct).then(data=>{res.json(data)}) 
...
router.post('/client',function(req,res){
    ...
    userService.updateStatus(req.body.accountId);
})
...
function verifyToken(req,res,next){
    ...
    var bearerHeader= req.headers["authorization"];
    console.log(bearerHeader+"myttttt");
    if(typeof bearerHeader!=='undefined'){
        const bearer= bearerHeader.split(" ");
        const bearerToken= bearer[1];
        req.token=bearerToken
...
router.post('/login',(req,res,next)=>{
  return  UserData.find({email:req.body.email}).exec().then(user=>{
...
router.post('/charge', function(req, res){
    var token2= req.body.stripeToken;
    var chargeAmount= req.body.chargeAmount;
    console.log(token2+" and "+chargeAmount)
    var charge= stripe.charges.create({
        amount:chargeAmount,
        currency: "USD",
      source:token2}, function(err, charge){
